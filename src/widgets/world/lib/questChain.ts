...
function refreshBubbles() {
  ...
  // 과일가게 사장: 과일·배달 단계 'dots' / 사과 다 따거나 배달 완료 보고 대기 'check'
  const vendorOn = stage === 'fruit' || stage === 'delivery';
  const vendorReport =
    (stage === 'fruit' && fruitQuestGiven && applesPicked >= FRUIT_GOAL) ||
    (stage === 'delivery' && deliveryReady);
  storyNpcs.vendor?.setBubble(vendorOn ? (vendorReport ? 'check' : 'dots') : false);
  // 큐비: trash 단계 'dots' / 다 버리고 보고 대기 'check'
  cube?.setBubble(stage === 'trash' ? (trashReady ? 'check' : 'dots') : false);
}
...
export function reportGallery(): void {
  if (stage !== 'gallery' || !galleryReady) return;
  galleryReady = false;
  goStage('fruit'); // refreshBubbles → 가람 off, 과일가게 사장 dots
  showQuestComplete('전시회 관람');
  setObjective('🍎 광장의 과일가게 사장과 이야기하자');
}
...
export function talkVendor(): void {
  ...
  // 아직 차례 아님 (경비/갤러리 단계) — 퀘스트 재촉 대신 일상 대화
  if (stage === 'guard' || stage === 'gallery') {
  ...
  if (stage === 'fruit') {
    if (!fruitQuestGiven) {
      ...
      openDialogue(VENDOR_PICK_GIVE, VENDOR_NAME, opts);
      return;
    }
    if (applesPicked < FRUIT_GOAL) { openDialogue(VENDOR_NEED_FRUIT, VENDOR_NAME, opts); return; }
    // 사과 3개 보고(수확 완료) → 완료 배너 → 대화가 끝나면 배달 타임어택 시작
    v?.setTalking(false);
  ...
  if (stage === 'delivery') {
    ...
    if (deliveryReady) {
      // 배달 완료 보고 → 완료 배너 → 큐비 단계로
      deliveryReady = false;
      goStage('trash'); // refreshBubbles → 사장 off, 큐비 dots
      showQuestComplete('신선 배달');
      setObjective('🧹 광장의 큐비에게 가서 마무리를 돕자');
      openDialogue([
        `등급 ${deliveryGrade} 배달, 정말 훌륭했어요! 덕분에 오늘 장사 잘했네요.`,
        '이제 광장의 큐비가 손이 필요할 거예요. 가서 도와주세요!',
      ], VENDOR_NAME, opts);
      return;
    }
    v?.setTalking(false);
    openDialogue(VENDOR_RETRY, VENDOR_NAME, { onEnd: startDelivery }); // 실패 후 재도전
    return;
  }

  openDialogue(VENDOR_AFTER, VENDOR_NAME, opts); // trash/ending/done
}

// 과수원 사과 줍기 — 안내 링이 켜진(=퀘스트 수락) 뒤에만 줍는다.
export function pickFruit(poi): void {
  if (stage !== 'fruit' || !fruitQuestGiven) {
...
export function trashUnlocked(): boolean {
  return stage === 'trash' || stage === 'ending' || stage === 'done';
}
// 큐비 퀘스트 수락 시 — 목표 갱신 (quest.ts 가 호출).
export function onTrashAccepted(): void {
  if (stage === 'trash') setObjective('🧹 길목의 쓰레기를 주워 광장 쓰레기통에 버리자');
}
...
// 큐비에게 완료 보고 후 — 엔딩(방명록) 단계로. quest.ts 가 보고 대화 끝에 호출.
export function onTrashDone(): void {
  if (stage === 'trash') {
    trashReady = false;
    goStage('ending'); // refreshBubbles → 큐비 풍선 OFF
    setObjective('✍️ 방명록 파티룸에서 흔적을 남기고 여정을 마치자!');
  }
}
...
function setObjective(text) {
  const el = document.getElementById('quest-objective');
  if (!el) return;
