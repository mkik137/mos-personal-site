...
character/player/  플레이어 애니메이션 GLB (+ Fire.glb 파이어볼 발사체)
character/npc/     NPC 캐릭터 (애니메이션 내장 GLB)
skill/             스킬 발사체 모델 (Meteor=혜성/운석·Bolt=번개 — fireball.ts MODELS 레지스트리)
...
player/             플레이어 캐릭터 (Mixamo 애니메이션) + fireball.ts(마법 발사체 — Fire.glb
                    풀, 셰이더 프리워밍으로 첫 발사 렉 제거, spawn(origin,dir,opts))
skills.ts           스킬 장착/시전 — 중앙하단 스킬바(#skillbar 4슬롯)·K 키 스킬창(#skill-window).
                    SKILL_CATALOG(발사 패턴/속도/색만 다른 변형) → 숫자키 1~4/F·클릭으로 시전.
                    마법 시전 모션(castMagic) + fireball 재사용. 의존성 주입(initSkills)
...
> `Minimap`(#minimap), `MapWindow`(#map-window), `InventoryPanel`(#inventory),
> `SkillBar`(#skillbar — 슬롯/열기버튼은 skills.ts 가 생성)·`SkillWindow`(#skill-window) — 모두 정적 DOM 을
> React 가 렌더하고 questChain/quest/inventory/skills 가 클래스·텍스트를 토글한다 (CSS 가 literal 셀렉터인 이유).
